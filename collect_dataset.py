...
class Listener:
    ...
    def __init__(self, args):
        ...
        self.NUM_SAMPLES = int(self.sample_rate * self.record_seconds)
...
def interactive(args):
    ...
    try:
        while True:
            ...
            print("recording number: ",index)
            input('press enter to continue. the recoding will be {} seconds. press ctrl + c to exit'.format(args.seconds))
            time.sleep(0.2)  # so the mic don't pick up clicking sound
            frames = [listener.stream.read(listener.NUM_SAMPLES, exception_on_overflow = False)]
    ...
    try:
        while True:
            if listener.record_seconds == None:  # record until keyboard interupt
                ...
                frames = [listener.stream.read(listener.chunk, exception_on_overflow = False)]
            else:
                frames = [listener.stream.read(listener.NUM_SAMPLES, exception_on_overflow = False)]
